...
const ExternalCompanies: React.FC = () => {
  ...
  const [listOptions, setListOptions] = useState({ pageNumber: 1, pageSize: 20 } as ListOptions);
        ...
        <Table compact unstackable color="orange">
          <Table.Header>
            ...
            <Responsive as={Table.Row} minWidth={LAPTOP_MEDIA_WIDTH}>
              ...
              <Table.HeaderCell>{t('externalCompanies.assignedPeople')}</Table.HeaderCell>
              <Table.HeaderCell>{t('externalCompanies.unassignedMembersCount')}</Table.HeaderCell>
            </Responsive>
            {/* Show short header labels if screen width smaller than tablet */}
            <Responsive as={Table.Row} maxWidth={LAPTOP_MEDIA_WIDTH - 1}>
              <Table.HeaderCell>{t('externalCompanies.name')}</Table.HeaderCell>
              <Table.HeaderCell>{t('externalCompanies.assignedPeopleShort')}</Table.HeaderCell>
              <Table.HeaderCell>{t('externalCompanies.unassignedMembersCountShort')}</Table.HeaderCell>
            </Responsive>
          </Table.Header>
          <Table.Body>{companiesCount === 0 ? (
            <Table.Row>
            ...
            <Table.Footer>
              <Table.Row>
                <Table.HeaderCell colSpan={columnCount}>
                  <Paginator
                    initialPageNumber={listOptions.pageNumber}
                    initialPageSize={listOptions.pageSize}
